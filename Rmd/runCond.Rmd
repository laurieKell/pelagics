...
#install_github('fishfollower/SAM/stockassessment', ref='components')
...
```{r, assessment-whb}
load("../data/inputs/WHB/run/model.RData")
whb=SAM2FLStock(fit)
whbR=FLBRP(whb,nyears=20)
params(whbR)[]=median(rec(whb))

save(whb,whbR,file="../data/om/whb.RData")
```
...
save(her,herR,file="../data/om/her.RData")
```

```{r, assessment-mac}
load("../data/inputs/MAC/model fit.RData")
mac =SAM2FLStock(fit.new)
mac=window(mac,end=2021)
macR=FLBRP(mac,nyears=20)
params(macR)[]=median(rec(mac))

save(mac,macR,file="../data/om/mac.RData")
```


```{r, retro-whb}
load("../data/inputs/WHB/run/retro.RData")
whbRet=FLStocks(llply(RETRO, SAM2FLStock)) 

save(whbRet,file="../data/om/whbRet.RData")
```
...
save(herRet,file="../data/om/herRet.RData")
```

```{r, retro-mac}
load("../data/inputs/MAC/retro.RData")
macRet=FLStocks(llply(ret,SAM2FLStock))

save(macRet,file="../data/om/macRet.RData")
```
...
```{r, errWhb, fig.height=8, fig.width=8}
...
```{r, errHer, fig.height=8, fig.width=8}
...
```{r, errMac, fig.height=8, fig.width=8}
